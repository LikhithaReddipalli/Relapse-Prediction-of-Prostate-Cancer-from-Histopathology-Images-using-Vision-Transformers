...
from src.models.modules.cnn_patch_model import CustomModel

# UKE dataset relapse prediction using cnn with multiple instance learning
...
class CNNSurvPatch:
    ...
    def training(self):
        ...
        if config.onlyEval == False:
            ...
            #pooling_layer1 = nn.AdaptiveAvgPool2d(1).to(config.device)

            pooling_layer1 = nn.AdaptiveAvgPool2d(1).to(config.device)
            pooling_layer2 = nn.AdaptiveAvgPool1d(1).to(config.device)
            linear_layer = nn.Linear(1280,1).to(config.device)

            model = nn.Sequential(*list(pretrained_model.features))#,pretrained_model.classifier[0] )


            model.classifier[1] = nn.Sequential(nn.AdaptiveAvgPool2d(1),
                                                nn.AdaptiveAvgPool1d(1),
                                                 nn.Linear(in_features=1280, out_features=1)).to(config.device)

            #features = nn.Sequential(*list(pretrained_model.features)).to(config.device)

            #pooling_layer1 = nn.AdaptiveAvgPool2d(1).to(config.device)
            #pooling_layer2 = nn.AdaptiveAvgPool1d(1).to(config.device) # You can also use nn.AdaptiveMaxPool1d(1) for max pooling
...
if __name__ == '__main__':
    ...
    file_name = os.path.join("C:\\Users\\likhi\\Desktop\\Thesis\\pc\\pc_code\\gleason_prediction\\Experiments" , time.strftime("%Y_%m_%d_%H_%M_%S"))
    os.mkdir(file_name)
    wandb.init(project = "pc_224_surv", dir=file_name, name = config.model + ":" + time.strftime("%Y_%m_%d_%H_%M_%S") )
    config_var = {}
    ...
    train = basic_vit_surv(file_name)
    #test = Test()
